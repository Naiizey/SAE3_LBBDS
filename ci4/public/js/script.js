        ...
        reader.addEventListener("load", function () {
            ...
            if (csv.split("\n")[0].split(";").length > entete.size) {
                ...
                if (en_trop.length > 0) {
                    ...
                    preview.innerHTML =
                        "ligne(s) avec trop d'éléments : " + check_item_nbr;
                    preview.style.color = "red";
                    //on centre le texte
                    preview.style.textAlign = "center";
                ...
                for (let i = 0; i < cells.length; i++) {
                    ...
                    if (lst_entete.includes(cells[i].trim())) {
                        lst_entete.splice(
                            lst_entete.indexOf(cells[i].trim()),
                            1
                        );
                    }
                }
                    ...
                    if (i === 0) {
                        ...
                        if (has_empty_cell(cells) > 0) {
                            ...
                            if (entete_missing.length > 0) {
                                preview.innerHTML =
                                    "entêtes manquantes : " + entete_missing;
                                preview.style.color = "red";
    ...
    reader.addEventListener("load", function () {
        ...
        for (let i = 0; i < 10 && i < lines.length; i++) {
            ...
            if (i === 0) {
                for (let j = 0; j < cells.length; j++) {
                    cells[j] =
                        "<span style='color:black'>" + cells[j] + "</span>";
                    let cell = row.insertCell(-1);
                    cell.innerHTML = cells[j];
                }
            } else {
...
function updateQuantite() {
    ...
    if (nbArt > 100) baliseQuant.textContent = "+99";
    else baliseQuant.textContent = nbArt;
}
...
function lstAvisSignalements() {
    ...
    for (let numLigne = 0; numLigne < lignes.length; numLigne++) {
        ...
        ligneA.addEventListener("click", () => {
            window.location.href = `${base_url}/produit/${idProduit}/${idAvis}#avis`;
        });
    }
}
...
function lstClients() {
        ...
        if (bannir) {
            buttonA.addEventListener("click", () => {
                ...
                document.getElementsByClassName(
                    "titreSanction"
                )[0].innerHTML = `Bannir le client n°${
                    lignes.item(numLigne).clientA
                } ?`;
                afficherSanctions();
                ...
                document
                    ...
                    .addEventListener("click", () => {
                        ...
                        lignes
                            .item(numLigne)
                            .addEventListener("click", liensLstClients);
                    });

                document
                    .getElementById("fermer")
                    .addEventListener("click", () => {
                        cacherSanctions();
                        lignes
                            .item(numLigne)
    ...
    function afficherTimeout(clientA) {
        ...
        document.getElementsByClassName(
            "titreSanction"
        )[1].innerHTML = `Bannir temporairement le client n°${clientA} ?`;
...
function cgu() {
    ...
    function affichageCGU(event) {
        ...
        if (mentionsLegales.style.display == "none") {
            ...
            document
                .getElementsByClassName("fermerCGU")
                .addEventListener("blur", affichageCGU); // Ferme les CGU lors du clic sur la croix en haut à droite de la div CGU
        }
    }
                ...
                if (self.form.elements[i].disabled == true) {
                    ...
                    if (self.form.elements[i].type == "password") {
                        ...
                        if (role == "client") {
                            ...
                            while (divCacheModifsCli.length) {
                                divCacheModifsCli[0].classList.remove(
                                    "cacheModifMdp"
                                );
                            }
                    ...
                    if (self.form.elements[i].type == "password") {
                        if (role == "client") {
                            ...
                            self.form.elements[i + 1].classList.add(
                                "cacheModifMdp"
                            );
                            self.form.elements[i + 2].value = "";
                            self.form.elements[i + 2].classList.add(
                                "cacheModifMdp"
                            );
                            ...
                            self.form.elements[i + 1].parentNode.classList.add(
                                "cacheModifMdp"
                            );
                            self.form.elements[i + 2].parentNode.classList.add(
                                "cacheModifMdp"
                            );
...
function cataloguePrice() {
    ...
    const rangeInput = document.querySelectorAll(".range-input input"),
        ...
        priceInput = document.querySelectorAll(
            ".price-range input:not(.range-input input)"
        ),
...
function boutonCliquable(bouton, action) {
    if (screen.width < 1200) {
        ...
        screen.width >= 1200 &&
        bouton.classList.contains("fermer-filtre")
    ) {
        bouton.addEventListener("click", () => {
...
function boutonCliquableTris(bouton, action) {
    if (screen.width < 1200) {
        ...
        screen.width >= 1200 &&
        bouton.classList.contains("fermer-tris")
    ) {
        bouton.addEventListener("click", () => {
...
function selectAll() {
    ...
    for (let box of boxes) {
        ...
        box.addEventListener("change", (e) => {
            ...
            let sousCats = document.querySelectorAll(
                "." +
                    getParentNodeTilClass(e.target).getAttribute("class") +
                    "~ .sous-categorie"
            );
            let checkboxes = [];
...
function barreProgression() {
    ...
    if (progressBar.value < 5) {
        // Nombre de points sur la barre de progression
    ...
    for (let index = 0; index < 5; index++) {
        ...
        if (index * 25 <= progressBar.value) {
            // Point vert si le pourcentage est supérieur au numéro du point multiplié par 25
...
var formAdresseConstructor = function () {
    ...
    this.form.elements["utilise_nom_profil"].addEventListener(
        "change",
        this.utiliserProfil
    );
    ...
    this.creerErreur = function (destination, message) {
        if (
            Array.from(destination.children).filter(
                (child) =>
                    Array.from(child.classList).includes("paragraphe-erreur") &&
    ...
    this.verifierNomEtPrenom = (event) => {
        ...
        if (selfTarget.validity.valueMissing) {
            this.creerErreur(
                selfTarget.parentNode.parentNode.parentNode,
                "Attention champ(s) vide(s)"
            );
        } else {
    ...
    Array.from(this.form.elements)
        .filter((elem) => {
            return (
                elem.required &&
                !Array.from(elem.parentNode.parentNode.classList).includes(
                    "nomPrenom"
                )
            );
        })
        .forEach((elemRequired) => {
            elemRequired.addEventListener("blur", (event) => {
                ...
                if (selfTarget.validity.valueMissing) {
                    this.creerErreur(
                        seekPositionErreur(selfTarget.name),
                        "Champ vide"
                    );
        ...
        if (this.codePostal.validity.patternMismatch) {
            this.creerErreur(
                document.querySelector(
                    `.position-erreur[for=${selfTarget.name}]`
                ),
                "Ne correspond à aucun code postal"
            );
        ...
        this.etoilet = function () {
            ...
            scrambleInterval = window.setInterval(() => {
                if (cursor <= $source.innerText.length) {
                    $target.innerText =
                        $source.innerText.substring(0, cursor) +
                        getRandomizedString($source.innerText.length - cursor);
                }
            }, 450 / 30);
...
function menuCredit() {
    ...
    lienConnexion.addEventListener("mouseout", () => {
        setTimeout(function () {
            ...
            if (!hover) {
                // verifie si le booléen est faux (ce qui veut dire que la souris n'est ni sur l'icône du profil ni sur le menu contextuel)
...
function setUpPaiment() {
    document.querySelector("[type='submit']").addEventListener("click", (e) => {
        e.preventDefault();
        let forms = [document.forms["form_paiement"]];
...
class AlerteAlizon {
    ...
    affichage = (message = this.message) => {
        document
            .querySelectorAll("main, header, footer")
            .forEach((element) => (element.style.filter = "blur(4px)"));
...
function avisProduit() {
    ...
    var bgfade = function () {
        ...
        if (avisEnValeur != null) {
            avisEnValeur.style.boxShadow =
                "0 0 10px rgba(0, 0, 0, " + bgopacity + ")";
    ...
    document
        .querySelector(".divProfilText input")
        .addEventListener("input", function () {
            document.querySelector(".divBoutonsComment").style.display = "flex";
        });

    document
        .querySelector(".divBoutonsComment button")
        .addEventListener("click", function () {
            etoiles.forEach((element) => {
                element.removeAttribute("class");
                document.querySelector(".divBoutonsComment").style.display =
                    "none";
                document.querySelector(".divEtoilesComment p").textContent =
                    "_/5";
                if (document.querySelector(".bloc-erreurs") != null) {
                    document.querySelector(".bloc-erreurs").style.display =
                        "none";
                }
            });
        });
    ...
    function validerEtoile() {
        ...
        for (let i = 0; i < etoiles.length; ++i) {
            ...
            if (etoiles[i] === this) {
                document.querySelector(".divEtoilesComment p").textContent =
                    i + 1 + "/5"; // écrit le numéro de la note à coté des étoiles
                document.querySelector(".inputInvisible").value = i + 1;
                document.querySelector(".divBoutonsComment div").style.cursor =
                    "auto";
                document.querySelector(".divBoutonsComment").style.display =
                    "flex";
...
function drapeauSignal(numAvis) {
    ...
    if (document.querySelectorAll(".drapeauSignal").length != null) {
        ...
        document
            .getElementsByClassName("drapeau" + numAvis)[0]
            .addEventListener("click", function () {
                document.getElementById(
                    "divSignalement" + numAvis
                ).style.display = "flex";
            });
    }
}
